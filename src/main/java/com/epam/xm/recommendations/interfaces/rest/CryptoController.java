...
public class CryptoController {
    ...
    @Operation(
            ...
            responses = {
                ...
                @ApiResponse(
                        responseCode = "404",
                        description = "Coin data not found",
                        content = @Content(schema = @Schema(implementation = ApiError.class))),
                @ApiResponse(
                        responseCode = "429",
    ...
    public List<CryptoRangeDto> getSortedRange() {
         ...
         */
        return cryptoService.getAllSortedStats().stream().map(cryptoMapper::toRangeDto).toList();
    }

    @Operation(
            summary = "Coin with the highest range for a specific day",
            description =
                    "Returns a single coin with the highest normalized range for the specified date",
            responses = {
                @ApiResponse(responseCode = "200", description = "Successful response"),
                @ApiResponse(
                        responseCode = "400",
                ...
                @ApiResponse(
                        responseCode = "404",
                        description = "No data for the specified date",
                        content = @Content(schema = @Schema(implementation = ApiError.class))),
                @ApiResponse(
                        responseCode = "429",
                        description = "Rate limit exceeded",
