...
public class RateLimitingTest extends com.epam.xm.recommendations.BaseIntegrationTest {
    ...
    void setup() {
        this.mockMvc =
                org.springframework.test.web.servlet.setup.MockMvcBuilders.webAppContextSetup(
                                context)
                        .addFilter(
                                new com.epam.xm.recommendations.infrastructure.config
                                        .RateLimitingFilter(
                                        context.getBean(
    ...
    void shouldReturn429WhenLimitExceeded() throws Exception {
        // First 10 requests should succeed (assuming they are supported symbols or at least return
        // some response)
        // Note: we don't mock anything, so it might return 400 or 404, which is still 2xx/4xx and
        ...
        mockMvc.perform(get("/api/v1/crypto/stats/BTC")).andExpect(status().isTooManyRequests());
    }
}
