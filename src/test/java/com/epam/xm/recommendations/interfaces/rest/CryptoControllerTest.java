...
public class CryptoControllerTest {
    ...
    void getStats_ShouldReturnOk_WhenValidSymbol() throws Exception {
        var stats = new CryptoStats("BTC", new BigDecimal("30000"), new BigDecimal("40000"), 
                new BigDecimal("30000"), new BigDecimal("45000"), new BigDecimal("0.5"));
        
        given(cryptoService.getStats("BTC")).willReturn(stats);
        given(cryptoMapper.toDto(any())).willReturn(new com.epam.xm.recommendations.application.dto.CryptoStatsDto("BTC", new BigDecimal("30000"), new BigDecimal("40000"), new BigDecimal("30000"), new BigDecimal("45000"), new BigDecimal("0.5")));
    ...
    void getStats_ShouldReturnBadRequest_WhenSymbolUnsupported() throws Exception {
        given(cryptoService.getStats("INVALID")).willThrow(new UnsupportedCryptoException("Unsupported"));
        
        mockMvc.perform(get("/api/v1/crypto/stats/INVALID"))
    ...
    void getHighestRange_ShouldReturnOk() throws Exception {
        ...
        given(cryptoService.getHighestRangeForDate(any(LocalDate.class))).willReturn(Optional.of(stats));
        // Use any() for DTO mapping if cryptoMapper is a MockitoBean
        given(cryptoMapper.toRangeDto(any())).willReturn(new com.epam.xm.recommendations.application.dto.CryptoRangeDto("BTC", new BigDecimal("0.5")));
